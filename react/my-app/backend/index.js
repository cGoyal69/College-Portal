...
async function run4(id,encryptyedPassword) { 
    let result = "Error";
    try {
        await client.connect();
        const database = client.db('studentdata');
        ...
        let s = new ObjectId(id);
        const query = { _id : s };
        result = await studentdata.updateOne({query},{$set:{password: encryptyedPassword},});
        console.log("Pinged your deployment. You successfully connected to MongoDB!");
    } catch (error) {
        console.error("Error connecting to MongoDB:", error);
    } finally {
...
app.post('/login', async (req, res) => {
    ...
    try {
        ...
        if(student) {
            jwt.sign({ user: student.username , pass: student.password}, JWT_SECRET, {
